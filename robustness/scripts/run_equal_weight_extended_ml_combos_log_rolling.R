...
extended_feature_sets <- c(base_feature_sets, har_augmented_feature_sets)

har_augmented_forecasts <- all_forecasts[
  target_type == "log" &
    window_type == "rolling" &
...
base_forecasts <- clean_forecasts[
  target_type == "log" &
    window_type == "rolling" &
    model_type %in% ml_methods &
...
extended_forecasts <- rbindlist(
  list(base_forecasts, har_augmented_forecasts),
  fill = TRUE
)
extended_forecasts <- unique(
  extended_forecasts,
  by = c("target_date", "forecast_id")
)
...
if (nrow(extended_forecasts) == 0L) {
  stop("No extended ML forecasts found for log/rolling feature sets.")
}
...
for (method in ml_methods) {
  member_ids <- extended_forecasts[model_type == method, sort(unique(forecast_id))]
  combo_specs[[paste0("combo_equal_weight_", method, "_across_extended_datasets_log_rolling")]] <- list(
    ids = member_ids,
    group = "within_ml_method_across_extended_datasets",
    family = method
  )
}

for (feature_set_value in extended_feature_sets) {
  member_ids <- extended_forecasts[feature_set == feature_set_value, sort(unique(forecast_id))]
...
combo_specs[["combo_equal_weight_all_extended_ml_x_dataset_log_rolling"]] <- list(
  ids = extended_forecasts[, sort(unique(forecast_id))],
  group = "all_extended_ml_x_dataset",
  family = "ALL_EXTENDED"
)
  ...
  lapply(
    ...
    function(combo_name) {
      ...
      make_equal_weight_combo(
        forecast_dt = extended_forecasts,
...
individual_forecasts <- extended_forecasts[target_date %in% common_dates]
...
combo_forecast_output <- file.path(
  config$paths$results_dir,
  "equal_weight_extended_ml_combos_log_rolling_forecasts.csv"
)
...
standard_eval_output <- file.path(
  config$paths$results_dir,
  "equal_weight_extended_ml_combos_log_rolling_standard_evaluation.csv"
)
...
message("Saved extended equal-weight combo forecasts to: ", combo_forecast_output)
message("Saved extended equal-weight combo evaluation vs HAR to: ", combo_eval_output)
message("Saved standard evaluation to: ", standard_eval_output)

cat("\nEXTENDED EQUAL-WEIGHT COMBINATIONS VS HAR\n")
