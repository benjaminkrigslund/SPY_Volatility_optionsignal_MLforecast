...
if (!file.exists(clean_forecast_path)) {
  stop(
    "Missing clean forecast panel: ", clean_forecast_path,
...
base_forecasts <- rbindlist(list(base_clean, har_augmented), fill = TRUE)
base_forecasts <- unique(base_forecasts, by = c("target_date", "forecast_id"))

base_forecasts <- base_forecasts[
  model_type %in% ml_methods &
    feature_set %in% extended_feature_sets
]
...
if (uniqueN(base_forecasts$forecast_id) != 35L) {
  warning(
    "Expected 35 base forecasts, found ", uniqueN(base_forecasts$forecast_id),
    ". Proceeding with available forecasts."
  )
}
...
run_stacked_forecasts <- function(stacking_dt,
  ...
  for (i in seq_len(nrow(stacking_dt))) {
    ...
    forecast_id <- paste0("stacked_", meta_model, "_on_35_extended_ml_forecasts_log_rolling")
...
stacked_runs <- lapply(
  meta_models,
  function(meta_model) {
    message("Running stacked meta-model on 35 forecasts: ", meta_model)
...
forecast_output <- file.path(
  config$paths$results_dir,
  "stacked_ml_on_35_extended_forecasts_log_rolling_forecasts.csv"
)
importance_output <- file.path(
  config$paths$results_dir,
  "stacked_ml_on_35_extended_forecasts_log_rolling_importance.csv"
)
...
standard_eval_output <- file.path(
  config$paths$results_dir,
  "stacked_ml_on_35_extended_forecasts_log_rolling_standard_evaluation.csv"
)
...
message("Saved stacked 35 forecasts to: ", forecast_output)
message("Saved stacked 35 importance to: ", importance_output)
message("Saved stacked 35 evaluation vs HAR to: ", eval_output)
message("Saved standard evaluation to: ", standard_eval_output)

cat("\nSTACKED ML ON 35 EXTENDED FORECASTS VS HAR\n")
print(vs_har_eval)

cat("\nBASE FORECAST IDS USED\n")
print(
  base_forecasts[
    ,
    .(forecast_id = sort(unique(forecast_id)))
  ]
)
