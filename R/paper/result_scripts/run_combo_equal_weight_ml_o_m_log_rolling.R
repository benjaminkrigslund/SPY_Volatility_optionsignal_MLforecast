...
if (length(common_targets) == 0L) {
  stop("No common out-of-sample target dates across O and M model forecasts.")
}
...
combo_specs <- list(
  combo_equal_weight_ml_o_log_rolling = sort(unique(aligned_forecasts[feature_set == "O", forecast_id])),
  combo_equal_weight_ml_m_log_rolling = sort(unique(aligned_forecasts[feature_set == "M", forecast_id])),
  combo_equal_weight_ml_o_m_log_rolling = sort(unique(aligned_forecasts$forecast_id))
)
...
forecast_output <- file.path(config$paths$results_dir, "combo_equal_weight_ml_o_m_log_rolling_forecasts.csv")
eval_output <- file.path(config$paths$results_dir, "combo_equal_weight_ml_o_m_log_rolling_evaluation.csv")
